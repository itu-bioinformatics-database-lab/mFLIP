...
from deep_metabolitics.config import all_generated_datasets_dir, aycan_full_data_dir, work_workbench_metabolights_multiplied_by_factors_dir
...
working_folder = work_workbench_metabolights_multiplied_by_factors_dir

ds_id_list = get_all_ds_ids(folder_path=working_folder)


for ds_id in tqdm(ds_id_list):
    _dest_path = working_folder / f"pathwayfluxminmax_{ds_id}.csv"
    if not _dest_path.exists():
        ...
        pathwayminmax_df.to_csv(working_folder / f"pathwayfluxminmax_{ds_id}.csv")
