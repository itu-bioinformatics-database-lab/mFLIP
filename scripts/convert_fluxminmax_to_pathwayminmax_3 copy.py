...
from deep_metabolitics.config import all_generated_datasets_dir, aycan_full_data_dir, data_dir
...
working_folder = data_dir / "reactionminmax_10_folds"

def get_all_ds_ids(folder_path):
    import os
    fname_list = [
        fname.replace("label_", "").replace(".parquet.gzip", "")
...
for ds_id in ds_id_list:
    ...
    fluxminmax_df = pd.read_parquet(working_folder / f"label_{ds_id}.parquet.gzip")
    X_breast_train = []
    ...
    pathwayminmax_df.to_parquet(working_folder / f"pathwayfluxminmax_{ds_id}.parquet.gzip", compression="gzip")
    del pathwayminmax_df
    del X_breast_train_transformed
    gc.collect()
