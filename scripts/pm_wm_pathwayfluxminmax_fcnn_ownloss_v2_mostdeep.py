...
from deep_metabolitics.networks.metabolite_fcnn_ownloss_v2 import MetaboliteFCNN
...
experiment_name = f"{experiment_name}_{metabolite_scaler_method}_{target_scaler_method}_{metabolite_coverage}_{k_folds}_{batch_size}_{epochs}_no_objective_loss"

# ids_list = get_workbench_metabolights_dataset_ids()
# print(f"{len(ids_list) = }")

test_source_list = get_aycan_dataset_ids()
# TODO 1 oluyorsa baslatacagiz
fold_list = [9]
print(f"{fold_list = }")
for fold in fold_list:
    ...
    if not model_file_path.exists():
        ...
        model = MetaboliteFCNN(
            metabolities=list(train_all_dataset.metabolomics_df.columns),
            label_names=list(train_all_dataset.label_df.columns),
            input_dim=n_features,
            output_dim=out_features,
            hidden_dims=[2048, 1024, 512, 256, 196],
