...
class MetaboliteFCNN(nn.Module):
    def __init__(
        ...
        updated_dropout_rate = dropout_rate
        for i in range(len(hidden_dims) - 1):
            # Boyut değiştiren layer
            updated_dropout_rate -= (0.1 * (i+1))
            if updated_dropout_rate < 0:
                updated_dropout_rate = 0
            dimension_change = nn.Sequential(
                nn.Linear(hidden_dims[i], hidden_dims[i + 1]),
                nn.LayerNorm(hidden_dims[i + 1]),
                nn.LeakyReLU(0.2),
                nn.Dropout(updated_dropout_rate),
            )
            ...
            residual_blocks = nn.ModuleList(
                [
                    ResidualBlock(hidden_dims[i + 1], updated_dropout_rate)
...
if __name__ == "__main__":
    ...
    # %%
    experiment_name = "fluxminmax_mns_optimization_fullbatch_bigger_impute"
