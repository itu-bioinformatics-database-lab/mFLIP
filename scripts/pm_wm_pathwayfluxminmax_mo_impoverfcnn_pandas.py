...
for fold in list(range(k_folds)):
    ...
    if not model_file_path.exists():
        _submodel = TorchRegressor(input_dim=n_features, hidden_dims=[1024, 128], dropout_rate=0.2, num_residual_blocks=0, epochs=epochs, batch_size=batch_size)
        model = MultiOutputRegressor(_submodel, n_jobs=15)
