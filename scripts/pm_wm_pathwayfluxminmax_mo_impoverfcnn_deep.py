...
from deep_metabolitics.networks.torch_regressor_impover import TorchRegressor
...
experiment_name = f"{experiment_name}_{metabolite_scaler_method}_{target_scaler_method}_{metabolite_coverage}_{k_folds}_{batch_size}_{epochs}"
print(f"{experiment_name = }")
...
for fold in list(range(k_folds)):
    ...
    if not model_file_path.exists():
        _submodel = TorchRegressor(input_dim=n_features, hidden_dims=[2048, 1024, 512, 256],
            num_residual_blocks=2,
            dropout_rate=0.2, epochs=epochs, batch_size=batch_size)
        model = MultiOutputRegressor(_submodel, n_jobs=5)
