...
experiment_name = "pm_wm_pathwayfluxminmax_gnn_pandas"
...
for fold in fold_list:
    ...
    for ds_name in test_source_list:
        print(f"{ds_name = }")
        test_all_dataset_t = PathwayFluxMinMaxDataset(
            dataset_ids=[ds_name],
            scaler_method=target_scaler_method,
            metabolite_scaler_method=metabolite_scaler_method,
            datasource="aycan",
        ...
        test_all_dataset = GraphDataset(dataset=test_all_dataset_t)
        ...
        print(f"{test_all_dataset_t.factors_df.shape = }")
        ...
        import pandas as pd
        pd.DataFrame(y_true, columns=list(train_all_dataset.label_df.columns)).to_csv(f"true_gnn_{ds_name}_{fold}.csv")
        pd.DataFrame(y_pred, columns=list(train_all_dataset.label_df.columns)).to_csv(f"pred_gnn_{ds_name}_{fold}.csv")
