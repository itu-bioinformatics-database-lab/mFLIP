...
for fold in list(range(k_folds)):
    ...
    # print(f"{df['target_name'] = }")
    # print(f"{df['target_value'] = }")
    # print(f"{df['bound_avg'] = }")
    # print(f"{df['reaction_count'] = }")
    # print(f"{df['pathway_name'] = }")
    # print(f"{df['row_idx'] = }")
    # print(f"{df['idx'] = }")


    train_all_dataset = OneoutDataset(dataframe_path=train_path, scaler=metabolite_scaler_method)
    # validation_all_dataset = OneoutDataset(dataframe_path=validation_path, scaler=train_all_dataset.scaler)
    test_all_dataset = OneoutDataset(dataframe_path=train_path, scaler=train_all_dataset.scaler)
    ...
    dataloader = DataLoader(
        train_all_dataset, batch_size=batch_size # , shuffle=False, num_workers=4, pin_memory=True, persistent_workers=True, prefetch_factor=2
    ...
    for batch in dataloader:
        ...
        print(f"{metabolites.shape = }")
        print(f"{pathway_targets.shape = }")
