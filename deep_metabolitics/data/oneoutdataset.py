...
class OneoutDataset(Dataset):
    def __init__(self, dataframe_path, scaler, one_hot_encoder=None):
        ...
        if isinstance(dataframe_path, DataFrame):
            ...
            # self.dataframe = spark.read.parquet(str(dataframe_path), inferSchema=True)
            print(f"{dataframe_path = }")
        print(f"before fillna {self.dataframe.count()}")
        ...
        if one_hot_encoder is None:
            ...
            one_hot_encoder.fit(self.dataframe.select("pathway_name").distinct().toPandas())
    ...
    def fit_scaler(self, scaler):
        ...
        #     .toPandasIterator(batchSize=10_000)  # her sefer 100k satır
        # )

        # def iter_partitions(df, cols):
        #     for part in df.select(cols).rdd.mapPartitions(lambda it: [pd.DataFrame(list(it), columns=cols)]).collect():
        #         yield part
        
        # def iter_partitions(df, cols, batch_size=1_000):
        #     # df = df.select(cols)
        #     # batch = []
        #     # for row in df.rdd.toLocalIterator():
        #     #     batch = self.dataframe.filter(F.col("idx").between(start_row, end_row)).toPandas()
        #     #     batch.append(row.asDict())
        #     #     if len(batch) >= batch_size:
        #     #         yield pd.DataFrame(batch)[cols]
        #     #         batch = []
        #     # if batch:
        #     #     yield pd.DataFrame(batch)[cols]
        ...
        #         batch = df.filter(F.col("idx").between(start_row, end_row)).toPandas()[cols]
        #         yield batch


        # pandas_iter = iter_partitions(self.dataframe, self.features)
        ...
        n_rows = self.dataframe.count()
        batch_size = 1_000
        for start_row in range(0, n_rows, batch_size):
            print(f"{start_row = }")
            end_row = start_row + batch_size
            batch = self.dataframe.filter(F.col("idx").between(start_row, end_row)).toPandas()[self.features]
            scaler.partial_fit(batch.values)  # pdf: sadece numeric_cols
        return scaler
    ...
    def __getitem__(self, idx):
        ...
        features = self.dataframe.filter(F.col("idx").between(start_row, end_row)).toPandas()
    ...
    def get_real_target(self):
        df = self.dataframe.select(["row_idx", "target_name", "target_value"]).toPandas()
    ...
    def get_predicted_target(self):
        df = self.dataframe.select(["row_idx", "target_name"]).toPandas()
        df["predicted"] = self.predicted
        _target = df.pivot_table(
            index="row_idx",
            columns="target_name",
            values="predicted",
