...
from deep_metabolitics.utils.utils import get_device, load_pathway_metabolites_map
...
class BaseDataset(torch.utils.data.Dataset):
    ...
    def preprocess(self):
        ...
        elif isinstance(self.metabolite_coverage, list):
            metabolites = self.metabolite_coverage
        else:
            metabolites = list(self.metabolomics_df.columns)
        if self.pathway_features:
        ...
        self.metabolites_feature_columns = metabolites
        print(f"{len(metabolites) = }")
            ...
            for column in self.metabolomics_df.columns:
                ...
                for p, metabolites in pathway_metabolites_map.items():
                    if column in metabolites:
                        metabolites = self.metabolomics_df.columns.intersection(
                            metabolites
                        )
                        self.metabolomics_df.loc[mask, column] += self.metabolomics_df[
                            mask
    ...
    def normalize_metabolites(self):
        if self.metabolite_scaler is None:
            ...
            if self.metabolite_scaler is not None:
                self.metabolite_scaler.fit(self.metabolomics_df[self.metabolomics_df.columns])
    ...
    #         #     row_min = np.min(features)  # Her satırın minimumu
    #         #     row_max = np.max(features)  # Her satırın maksimumu
    #         #     features = (features - row_min) / (
    ...
    def generate_image(row):
        ...
        recon_metabolites = list(row.index)
        # print(f"{len(recon_metabolites) = }")
        image = np.full((len(pathway_metabolites_map), len(recon_metabolites)), 0.0)
    ...
    def __getitem__(self, index):
        ...
        row = self.metabolomics_df.iloc[index]
        features = self.generate_image(row=row)
        label = self.label_df.iloc[index].values
...
class ReactionMinMaxDataset(BaseDataset):
    ...
    def load_data_from_folder(
        ...
        **kwargs,
        ...
        factors_df = pd.read_parquet(
            input_dir
            / f"factors_{dataset_id}.parquet.gzip"
        )[["Factors"]]
        ...
        label_df = pd.read_parquet(
            input_dir
            / f"label_{dataset_id}.parquet.gzip"
        )

        dataset_ids_df = pd.read_parquet(
            input_dir
            / f"dataset_ids_{dataset_id}.parquet.gzip"
        )
...
class PathwayFluxMinMaxDataset(BaseDataset):
    ...
    def source_load_data(
        ...
        **kwargs,
        ...
        if datasource == "csv":
            ...
            metabolomics_df, label_df, dataset_ids_df, factors_df = (
                PathwayFluxMinMaxDataset.load_data_from_folder(
                    dataset_ids=dataset_ids, **kwargs
                )
            )
        else:
    ...
    def load_data_from_folder(
        ...
        **kwargs,
        ...
        dataset_ids_df = dataset_ids_df.reset_index(drop=True)
        factors_df = factors_df.reset_index(drop=True)

        filter_null_columns_by =  kwargs.get("filter_null_columns_by", None)
        filter_null_rows_by =  kwargs.get("filter_null_rows_by", None)

        if filter_null_columns_by is not None:
            metabolomics_df = metabolomics_df.loc[:, metabolomics_df.isna().mean() < filter_null_columns_by]

        if filter_null_rows_by is not None:
            mask = metabolomics_df.isna().mean(axis=1) <= filter_null_rows_by
            metabolomics_df = metabolomics_df[mask]
            label_df = label_df[mask]
            dataset_ids_df = dataset_ids_df[mask]
            factors_df = factors_df[mask]
            print(f"{metabolomics_df.shape = }")
