...
def train_pyspark(
    ...
    X_train, y_train = train_dataset.df, train_dataset.label_df
...
def train_own_dnn_oneepoch(dataloader, model, optimizer, device):
    model.train()
    for batch in tqdm(dataloader):
        metabolites, pathway_targets = batch
        metabolites = metabolites.to(device)
        pathway_targets = pathway_targets.to(device)
        metabolites = metabolites.to(device=device)
        pathway_targets = pathway_targets.to(device=device)
        ...
        optimizer.step()
    print(f"{loss_dict['loss'] = }")
    return model, optimizer


def train_pytorch_tabular(train_dataset, model, validation_df):
    start_time = time.time()
    model.fit(train=train_dataset, validation=validation_df)
    end_time = time.time()
    train_elapsed_time = end_time - start_time
...
def train_own_dnn(
    ...
    epochs=200,
    is_graph=False,
):  # TODO early stopping, kullanilmiyor, ileriye donuk belki ekleriz,
    if is_graph:
    ...
    )  # TODO burasi optimize degil

    # dataloader = DataLoader(
    #     train_dataset, batch_size=batch_size, shuffle=False, pin_memory=True, num_workers=4, persistent_workers=True, prefetch_factor=4
    # )  # TODO burasi optimize
    ...
    for epoch in tqdm(range(epochs)):
        ...
        # Training phase
        model.train()
        model, optimizer = train_own_dnn_oneepoch(
            dataloader=dataloader, model=model, optimizer=optimizer, device=device
        )
    end_time = time.time()
    train_elapsed_time = end_time - start_time
...
def predict_sklearn(model, dataset):
    ...
    return all_predictions, y, elapsed_time


def predict_pyspark(model, dataset):
    X, y = dataset.df, dataset.label_df
    start_time = time.time()
    all_predictions = model.predict(X)
    end_time = time.time()
    elapsed_time = end_time - start_time
    y = model.label_tonumpy(y)

    return all_predictions, y, elapsed_time



def predict_pytorch_tabular(model, X, y):
    start_time = time.time()
    all_predictions = model.predict(X)
    end_time = time.time()
    elapsed_time = end_time - start_time

    all_predictions.columns = [c.replace("_prediction", "") for c in all_predictions.columns]
    all_predictions = all_predictions[y.columns]

    return all_predictions.values, y.values, elapsed_time


def predict_own_dnn(model, dataset, device="cuda", test_batch_size=64, is_graph=False):
    if is_graph:
        from torch_geometric.loader import DataLoader
    else:
        from torch.utils.data import DataLoader
