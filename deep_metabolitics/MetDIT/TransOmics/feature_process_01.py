...
def parser_args():
    parser = argparse.ArgumentParser(
        description="Data Pre-process for TransOmics with Python"
    )
    parser.add_argument(
        "-ofp",
        "--original_file_path",
...
def feature_ranking(
    frames, method_type="RF", file_name="sample", vis=True, vis_num=None, args=None
):
    """
    ...
    if method_type == "RF":
        ...
        raise NotImplementedError(
            "Feature important ranking not support {} now.".format(method_type)
        )
    ...
    feature_imp = pd.DataFrame(
        sorted(zip(clf.feature_importances_, feature_name)),
    ...
    if vis_num is None:
        ...
        raise ValueError(
            "Feature Visualization Number is larger than the total feature number of input samples."
        )
    ...
    if vis:
        ...
        sns.barplot(
            x="Value",
            y="Feature",
            data=feature_imp.sort_values(by="Value", ascending=False)[:vis_num],
        )
        plt.title("LightGBM Features (avg over folds)")
        plt.tight_layout()
        vis_save_path = os.path.join(
            args.save_file_path, "{}_importances_{}.png".format(method_type, file_name)
        )
        plt.savefig(vis_save_path)
        plt.show()

    feature_imp = feature_imp.sort_values(by="Value", ascending=False)
    rec_save_path = os.path.join(
        args.save_file_path, "{}_importances_{}.csv".format(method_type, file_name)
    )
...
def data_normalization(frames, log_func=True, norm_func="minmax"):
    ...
    if norm_func == "minmax":
        ...
        raise NotImplementedError(
            "Not support Normalization of {}".format(args.norm_func)
        )
...
def run_command(args):
    ...
    """
    frames = load_csv(
        args.original_file_path, args.rate
    )  # frame@dataframe : horizontal data type, each row defines a ID sample.

    # data normalization
    norm_frames = data_normalization(
        frames, log_func=args.log_func, norm_func=args.norm_func
    )
    ...
    top_feature_names = feature_ranking(
        norm_frames,
