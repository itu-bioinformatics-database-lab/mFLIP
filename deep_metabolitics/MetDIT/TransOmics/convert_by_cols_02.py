...
def extented_feature(frames, img_sz):
    ...
    if hi < img_sz:
        ...
        for idx in range(ele):
            res = np.vstack((res, frames))  # channel olusturuyor.
...
def generate_function(method_type, img_sz, save_path, file_name):
    ...
    label = (
        src_data.columns.values.astype(float).astype(int).astype(str)
    )  # burada ne yapiyor anlamadim
    ...
    src_data_T = src_data.T
    where_are_NaNs = np.isnan(src_data_T)
    src_data_T[where_are_NaNs] = 0
    gaf_images = gaf.fit_transform(src_data_T)
    ...
    for idx in tqdm(range(img_num)):
        ...
        img_save_path = os.path.join(
            save_path, "images", label[idx], "{}.png".format(str(idx))
        )
        image.imsave(img_save_path, gaf_img)

        data_save_path = os.path.join(
            save_path, "data", label[idx], "{}.csv".format(str(idx))
        )
