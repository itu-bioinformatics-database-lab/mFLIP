...
class MetaboliteVAEWithFCNN(nn.Module):
    def __init__(
        self,
        ...
        if fold is None:
            fcnn_model = MetaboliteFCNN(input_dim=metabolite_dim, output_dim=pathway_dim)
            fcnn_model = fcnn_model.to(device="cuda" if torch.cuda.is_available() else "cpu")
            fcnn_model = warmup_training(fcnn_model, train_dataset, num_warmup_steps=100)
        else:
            from deep_metabolitics.config import outputs_dir
            import joblib
            experiment_name = f"pm_wm_pathwayfluxminmax_fcnn_pandas_mostdeep_std_std_None_10_256_fold_{fold}"
            fcnn_model_file_path = outputs_dir / f"{experiment_name}.joblib"
            fcnn_model = joblib.load(fcnn_model_file_path)
            fcnn_model = fcnn_model.to(device="cuda")
        ...
        # Output layers
        self.metabolite_output = nn.Linear(decoder_dims[-1], metabolite_dim)
