...
class MetaboliteFCNN(nn.Module):
    ...
    def loss_function(self, predictions, targets):
        ...
        # pred_softmax = F.softmax(predictions, dim=0)
        # target_softmax = F.softmax(targets, dim=0)

        # Cross Entropy loss hesaplama
        # ce_loss = F.binary_cross_entropy(pred_softmax, target_softmax)
        ce_loss = F.binary_cross_entropy_with_logits(predictions, targets)

        # log_cosh_loss = torch.mean(torch.log(torch.cosh(predictions - targets)))

        total_loss = ce_loss + mse_loss + l1_loss
        # + l1_loss + log_cosh_loss
        ...
        #     + ce_loss  # Cross Entropy loss eklendi
