...
class MetaboliteVAE(nn.Module):
    def __init__(
        ...
        print(f"!!! {metabolite_dim = }")
        print(f"!!! {pathway_dim = }")
        total_input_dim = metabolite_dim + pathway_dim
        print(f"{total_input_dim = }")
    ...
    def encode(self, x):
        ...
        log_var = torch.clamp(log_var, min=-10, max=10)  # veya daha dar bir aralık
        return mu, log_var
    ...
    def forward(self, x):
        pathway_features = torch.zeros((len(x), self.pathway_dim), device=x.device)
    ...
    def loss_function(
        ...
        # import numpy as np
        # print(f"{np.sum(np.isnan(pathways_pred.detach().cpu().numpy())) = }")
        # print(f"{np.sum(np.isinf(pathways_pred.detach().cpu().numpy())) = }")
